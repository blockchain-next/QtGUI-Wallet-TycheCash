...
public:
  ...
  void start(Node** _node, const TycheCash::Currency* currency,  INodeCallback* _callback, Logging::LoggerManager* _loggerManager,
    ...
    try {
      ...
      Q_EMIT nodeInitFailedSignal(TycheCash::error::INTERNAL_WALLET_ERROR);
      QCoreApplication::processEvents();
      return;
    }
...
NodeAdapter::NodeAdapter() : QObject(), m_node(nullptr), m_nodeInitializerThread(), m_nodeInitializer(new InProcessNodeInitializer) {
  ...
  qRegisterMetaType<TycheCash::CoreConfig>("TycheCash::CoreConfig");
  qRegisterMetaType<TycheCash::NetNodeConfig>("TycheCash::NetNodeConfig");
...
TycheCash::IWalletLegacy* NodeAdapter::createWallet() const {
  Q_CHECK_PTR(m_node);
...
bool NodeAdapter::init() {
  Q_ASSERT(m_node == nullptr);
  QUrl localNodeUrl = QUrl::fromUserInput(QString("127.0.0.1:%1").arg(TycheCash::RPC_DEFAULT_PORT));
...
bool NodeAdapter::initInProcessNode() {
  ...
  TycheCash::CoreConfig coreConfig = makeCoreConfig();
  TycheCash::NetNodeConfig netNodeConfig = makeNetNodeConfig();
...
TycheCash::CoreConfig NodeAdapter::makeCoreConfig() const {
  TycheCash::CoreConfig config;
...
TycheCash::NetNodeConfig NodeAdapter::makeNetNodeConfig() const {
  TycheCash::NetNodeConfig config;
  boost::program_options::variables_map options;
