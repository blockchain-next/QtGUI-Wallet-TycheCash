...
typedef QPair<TycheCash::TransactionId, TycheCash::TransferId> TransactionTransferId;
...
private:
  ...
  QHash<TycheCash::TransactionId, QPair<quint32, quint32> > m_transactionRow;
  ...
  QVariant getUserRole(const QModelIndex& _index, int _role, TycheCash::TransactionId _transactionId, TycheCash::WalletLegacyTransaction& _transaction,
    TycheCash::TransferId _transferId, TycheCash::WalletLegacyTransfer& _transfer) const;

  void reloadWalletTransactions();
  void appendTransaction(TycheCash::TransactionId _id, quint32& _row_count);
  void appendTransaction(TycheCash::TransactionId _id);
  void updateWalletTransaction(TycheCash::TransactionId _id);
