...
TransactionsModel::TransactionsModel() : QAbstractItemModel() {
  ...
  connect(&WalletAdapter::instance(), &WalletAdapter::walletTransactionCreatedSignal, this,
    static_cast<void(TransactionsModel::*)(TycheCash::TransactionId)>(&TransactionsModel::appendTransaction), Qt::QueuedConnection);
...
QVariant TransactionsModel::data(const QModelIndex& _index, int _role) const {
  ...
  TycheCash::TransactionId transactionId = m_transfers.value(_index.row()).first;
  TycheCash::TransferId transferId = m_transfers.value(_index.row()).second;

  if(!WalletAdapter::instance().getTransaction(transactionId, transaction) ||
    (m_transfers.value(_index.row()).second != TycheCash::WALLET_LEGACY_INVALID_TRANSFER_ID &&
...
QVariant TransactionsModel::getUserRole(const QModelIndex& _index, int _role, TycheCash::TransactionId _transactionId,
  TycheCash::WalletLegacyTransaction& _transaction, TycheCash::TransferId _transferId, TycheCash::WalletLegacyTransfer& _transfer) const {
  switch(_role) {
  case ROLE_DATE:
  ...
  case ROLE_AMOUNT:
    return static_cast<qint64>(_transferId == TycheCash::WALLET_LEGACY_INVALID_TRANSFER_ID ? _transaction.totalAmount : -_transfer.amount);
  ...
  case ROLE_NUMBER_OF_CONFIRMATIONS:
    return (_transaction.blockHeight == TycheCash::WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT ? 0 :
...
void TransactionsModel::reloadWalletTransactions() {
  ...
  quint32 row_count = 0;
  for (TycheCash::TransactionId transactionId = 0; transactionId < WalletAdapter::instance().getTransactionCount(); ++transactionId) {
...
void TransactionsModel::appendTransaction(TycheCash::TransactionId _transactionId, quint32& _insertedRowCount) {
  TycheCash::WalletLegacyTransaction transaction;
  ...
  if (transaction.transferCount) {
    ...
    for (TycheCash::TransferId transfer_id = transaction.firstTransferId;
    ...
    m_transfers.append(TransactionTransferId(_transactionId, TycheCash::WALLET_LEGACY_INVALID_TRANSFER_ID));
    m_transactionRow[_transactionId] = qMakePair(m_transfers.size() - 1, 1);
    ++_insertedRowCount;
  }
}

void TransactionsModel::appendTransaction(TycheCash::TransactionId _transactionId) {
...
void TransactionsModel::updateWalletTransaction(TycheCash::TransactionId _id) {
