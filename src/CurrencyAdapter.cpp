...
#include "TycheCashWalletConfig.h"
...
CurrencyAdapter::CurrencyAdapter() : m_currency(TycheCash::CurrencyBuilder(LoggerAdapter::instance().getLoggerManager()).currency()) {
}

CurrencyAdapter::~CurrencyAdapter() {
}

const TycheCash::Currency& CurrencyAdapter::getCurrency() {
  return m_currency;
}
...
QString CurrencyAdapter::getCurrencyName() const {
  return TycheCash::TycheCash_NAME;
}
...
bool CurrencyAdapter::validateAddress(const QString& _address) const {
  TycheCash::AccountPublicAddress internalAddress;
