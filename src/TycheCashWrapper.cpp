...
bool parsePaymentId(const std::string& payment_id_str, Crypto::Hash& payment_id) {
  return TycheCash::parsePaymentId(payment_id_str, payment_id);
}
...
std::string convertPaymentId(const std::string& paymentIdString) {
  ...
  TycheCash::BinaryArray extraNonce;
  TycheCash::setPaymentIdToTransactionExtraNonce(extraNonce, paymentId);
...
std::string extractPaymentId(const std::string& extra) {
  std::vector<TycheCash::TransactionExtraField> extraFields;
  std::vector<uint8_t> extraVector;
  std::copy(extra.begin(), extra.end(), std::back_inserter(extraVector));

  if (!TycheCash::parseTransactionExtra(extraVector, extraFields)) {
    throw std::runtime_error("Can't parse extra");
  }

  std::string result;
  TycheCash::TransactionExtraNonce extraNonce;
  if (TycheCash::findTransactionExtraFieldByType(extraFields, extraNonce)) {
    Crypto::Hash paymentIdHash;
    if (TycheCash::getPaymentIdFromTransactionExtraNonce(extraNonce.nonce, paymentIdHash)) {
...
class RpcNode : TycheCash::INodeObserver, public Node {
public:
  RpcNode(const TycheCash::Currency& currency, INodeCallback& callback, const std::string& nodeHost, unsigned short nodePort) :
    m_callback(callback),
    m_currency(currency),
  ...
  TycheCash::IWalletLegacy* createWallet() override {
    return new TycheCash::WalletLegacy(m_currency, m_node);
  }

private:
  INodeCallback& m_callback;
  const TycheCash::Currency& m_currency;
  TycheCash::NodeRpcProxy m_node;
...
class InprocessNode : TycheCash::INodeObserver, public Node {
public:
  InprocessNode(const TycheCash::Currency& currency, Logging::LoggerManager& logManager, const TycheCash::CoreConfig& coreConfig,
    const TycheCash::NetNodeConfig& netNodeConfig, INodeCallback& callback) :
    ...
    m_core.set_TycheCash_protocol(&m_protocolHandler);
    m_protocolHandler.set_p2p_endpoint(&m_nodeServer);
    TycheCash::Checkpoints checkpoints(logManager);
    for (const TycheCash::CheckpointData& checkpoint : TycheCash::CHECKPOINTS) {
  ...
  void init(const std::function<void(std::error_code)>& callback) override {
    try {
      if (!m_core.init(m_coreConfig, TycheCash::MinerConfig(), true)) {
...
private:
  ...
  TycheCash::CoreConfig m_coreConfig;
  TycheCash::NetNodeConfig m_netNodeConfig;
  TycheCash::core m_core;
  TycheCash::TycheCashProtocolHandler m_protocolHandler;
  TycheCash::NodeServer m_nodeServer;
  TycheCash::InProcessNode m_node;
